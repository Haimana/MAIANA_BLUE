...
#if BOARD_REV==113 || BOARD_REV==114
...
#define EEPROM_TX_CONFIG_ADDRESS        0x70
#define STATION_DATA_FLASH_ADDRESS      0x0800F800

const char *BSP_HW_REV = "11.9.3";
...
void bsp_hw_init()
{
  ...
  // UART1 (main UART) - init UART first so we can log (NO I2C PIN CONFIG BEFORE THIS!)
  ...
  // UART1 is now ready for logging
  ...
  // Reconfigure pins back to I2C Alternate Function Open Drain before I2C init
  GPIO_InitTypeDef gpio_i2c;
  gpio_i2c.Pin = I2C_SCL_PIN;
  gpio_i2c.Mode = GPIO_MODE_AF_OD;
  gpio_i2c.Pull = GPIO_PULLUP;
  gpio_i2c.Speed = GPIO_SPEED_HIGH;
  gpio_i2c.Alternate = GPIO_AF4_I2C1;
  HAL_GPIO_Init(I2C_SCL_PORT, &gpio_i2c);
  
  gpio_i2c.Pin = I2C_SDA_PIN;
  ...
  // Delay to allow EEPROM to stabilize after I2C initialization
  HAL_Delay(100);  // 100ms delay for EEPROM to be ready
  ...
  // TEMPORARY: Skip EEPROM read in bsp_hw_init() - will be read later when needed
  memset(&__station, 0, sizeof(StationData));
}
...
void SystemClock_Config()
{
...
#if 0 // defined(STM32L432xx) || defined(STM32L431xx) || defined(STM32L422xx)
  ...
  // I2C clock configuration disabled for hardware debugging
...
void bsp_read_station_data(StationData *data)
{
  ...
  // Delay before EEPROM access to ensure it's ready
  HAL_Delay(50);
...
void bsp_read_tx_config(TXConfig *c)
{
  // Delay before EEPROM access to ensure it's ready
  HAL_Delay(50);
  
  uint8_t *d = (uint8_t*)c;
  for ( uint8_t i = 0; i < sizeof (TXConfig); ++i, ++d )
    {
      HAL_I2C_Mem_Read(&hi2c1, EEPROM_ADDRESS|1, i+EEPROM_TX_CONFIG_ADDRESS, 1, d, 1, 5);
    }
}

void bsp_write_tx_config(const TXConfig &c)
{
  TXConfig __c = c;
  uint8_t *d = (uint8_t*)&__c;
  for ( uint8_t i = 0; i < sizeof (TXConfig); ++i, ++d )
    {
      HAL_I2C_Mem_Write(&hi2c1, EEPROM_ADDRESS, i+EEPROM_TX_CONFIG_ADDRESS, 1, d, 1, 5);
      HAL_Delay(5);
    }
}

void bsp_erase_tx_config()
{
  TXConfig c = {0, 0, {0}};
  bsp_write_tx_config(c);
}
